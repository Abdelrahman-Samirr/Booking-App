...
function Recommended() {
    ...
    useEffect(() => {
        axiosInterceptor.get("recommended_hotels")
            .then((res) => {
                setData(res.data)
                ...
                {data?.length > 0 ? data.map((data) => (
                    <div key={data.id}>
                        <div className='shadow-md flex items-center !px-4.5 !py-3.5 !mx-5 rounded-xl bg-white gap-4'>
                            <figure className='w-[40%] rounded-md overflow-hidden cursor-pointer' onClick={() => handleCardClick(data)}><img className='w-full' src={data.images.main} alt="Hotel" /></figure>
                            <div className='W-[70%]'>
                                <div className='!mb-4'>
                                    <p className='text-[#596181] font-semibold text-xs'>Hotel</p>
                                    <h4 className='text-[#475073] font-extrabold text-base !my-2'>{data.name}</h4>
                                    <p className='text-[#596181] font-semibold text-xs'>{data.address.street},{data.address.country}</p>
                                </div>
                                <div className='flex'>
                                    <p className='text-[#6C738F] !mr-8 text-xs !mt-1.5'>Cupon: <span className='text-[#475073] font-bold'>DHSGJAB09D</span></p>
